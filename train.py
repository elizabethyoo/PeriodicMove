...
def train(model, optimizer, criterion, train_dataloader, config, verbose=True, validation=True, valid_dataloader=None, test_dataloader=None, dist_index_dict=None):
    # Check if the checkpoint exists and load it if it does
    if os.path.exists(config.save_path):
        print("Loading checkpoint from:", config.save_path)
        model.load_state_dict(torch.load(config.save_path))
    else:
        print("No checkpoint found at", config.save_path, ". Starting training from scratch.")
        ...
        for s in slices:
            ...
            if config.dist_loss:
                ...
                if epoch < 1000:
                    ...
                    train_dist_loss += alpha * dist_loss.item()
                else:
                    loss = recover_loss
